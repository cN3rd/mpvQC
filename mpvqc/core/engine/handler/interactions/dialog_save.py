#  mpvQC
#
#  Copyright (C) 2020 mpvQC developers
#
#  This program is free software: you can redistribute it and/or modify
#  it under the terms of the GNU Affero General Public License as published by
#  the Free Software Foundation, either version 3 of the License, or
#  (at your option) any later version.
#
#  This program is distributed in the hope that it will be useful,
#  but WITHOUT ANY WARRANTY; without even the implied warranty of
#  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
#  GNU Affero General Public License for more details.
#
#  You should have received a copy of the GNU Affero General Public License
#  along with this program.  If not, see <http://www.gnu.org/licenses/>.


from pathlib import Path
from typing import Optional

from mpvqc import get_settings
from mpvqc.core.engine.handler.save_path_suggester import SavePathSuggester
from mpvqc.core.engine.interface import App
from mpvqc.gui.filedialogs import Dialogs


class SaveDialog:

    def __init__(self, app: App):
        self._parent = app.parent()
        self._video: Optional[Path] = None

    def set_video(self, video: Optional[Path]):
        self._video = video

    def get_write_path(self) -> Optional[Path]:
        path_suggestion = self._get_suggested_file_path()
        return Dialogs.export_document(path_suggestion, self._parent)

    def _get_suggested_file_path(self) -> Path:
        s = get_settings()
        return SavePathSuggester.suggest(for_video=self._video, and_writer=s.export_nickname)
