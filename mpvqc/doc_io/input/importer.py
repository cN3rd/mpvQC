#  mpvQC
#
#  Copyright (C) 2020 mpvQC developers
#
#  This program is free software: you can redistribute it and/or modify
#  it under the terms of the GNU Affero General Public License as published by
#  the Free Software Foundation, either version 3 of the License, or
#  (at your option) any later version.
#
#  This program is distributed in the hope that it will be useful,
#  but WITHOUT ANY WARRANTY; without even the implied warranty of
#  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
#  GNU Affero General Public License for more details.
#
#  You should have received a copy of the GNU Affero General Public License
#  along with this program.  If not, see <http://www.gnu.org/licenses/>.


from pathlib import Path
from typing import Tuple, List

from mpvqc.doc_io import DocumentImport, Document, Incompatible
from mpvqc.doc_io.input.error import NotADocumentError
from mpvqc.doc_io.input.reader import FileReader


class Importer:
    EMPTY_IMPORT = DocumentImport(documents=tuple(), incompatibles=tuple())

    def __init__(self, files: Tuple[Path]):
        self._files = files
        self._documents: List[Document] = []
        self._incompatibles: List[Incompatible] = []

    def import_them(self) -> None:
        for file in self._files:
            self._read(file)

    def _read(self, file: Path):
        try:
            document = self._open(file)
            self._documents.append(document)
        except NotADocumentError as error:
            incompatible = error.get_incompatible()
            self._incompatibles.append(incompatible)

    @staticmethod
    def _open(file):
        reader = FileReader(file)
        reader.read()
        return reader.get_document()

    def get_import(self):
        return DocumentImport(
            documents=tuple(self._documents),
            incompatibles=tuple(self._incompatibles)
        )
